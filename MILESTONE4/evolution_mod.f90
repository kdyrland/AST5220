module evolution_mod
    ...
    ! hires variables
    integer(i4b), parameter     :: nhires = 5000

contains


  ! ----------- hires source function -----------
subroutine get_hires_source_function(xhires, khires, S)
    implicit none

    real(dp), pointer, dimension(:),   intent(out) :: xhires, khires
    ...
    ! x and k arrays
    allocate(xhires(nhires))
    allocate(khires(nhires))

    do i = 1,nhires
        xhires(i) = x_init - x_init * (i-1.d0)/(nhires-1.d0)
        khires(i) = kmin  + (kmax - kmin) * (i-1.d0)/(nhires-1.d0)
    end do

    allocate(S_lores(nt,nk))            ! low res source function
    allocate(S_coeff(4,4,nt,nk))
    allocate(S(nhires,nhires))          ! high res source function
    ...
    do i = 1,nt
        ...
        do k = 1,nk
            ck = c*ks(k)
            Pi    = Theta(i,2,k)        ! without polarization
            dPi   = dTheta(i,2,k)

            ddPi  = 2.d0 * ck/(5.d0*Hp) * (-dHp/Hp*Theta(i,1,k) + dTheta(i,1,k)) &
                    + 0.3d0 * (ddt*Pi+dt*dPi) &
                    - 3.d0 * ck/(5.d0*Hp) * (-dHp/Hp*Theta(i,3,k) + dTheta(i,3,k))


            S_lores(i,k) = g * (Theta(i,0,k) + Psi(i,k) + Pi/4.d0) &
                            + exp(-tau) &
                            * (dPsi(i,k) - dPhi(i,k)) &
                            - 1.d0/ck * (Hp*(g*dv_b(i,k) + v_b(i,k)*dg) + g*v_b(i,k)*dHp) &
                            + .75d0/ck**2 * ((H_0**2/2.d0*((omega_m+omega_b)/exp(xf(i)) &
                            + 4.d0*omega_r/exp(2.d0*xf(i)) + 4.d0*omega_v*exp(2.d0*xf(i)))) &
                            * g*Pi + 3.d0*Hp*dHp*(dg*Pi+g*dPi) + Hp**2 &
                            * (ddg*Pi + 2.d0*dg*dPi+g*ddPi))
        end do
    end do

    ! 2d spline
    call splie2_full_precomp(xf, ks, S_lores, S_coeff)
    ...
    do k = 1,nhires
        do i = 1,nhires
            S(i,k) = splin2_full_precomp(xf, ks, S_coeff, xhires(i), khires(k))
        end do
    end do
